...
*/
//linkObj.relation = function(link, url_parsed) //before
linkObj.relation = function(link, url_parsed, options) //after
{
	...
	else
	{
		...
		//baseRelation = urlobj.relation(urlobj.parse(options.firstPageUrl), link.base.parsed); //add

		link.baseInternal = link.base.parsed.host === urlobj.parse(options.firstPageUrl).host //add
		
		// link.testobj = {
		// 	url_parsed, 
		// 	base_parsed: link.base.parsed, 
		// 	relation, 
		// 	auth: urlobj.component.AUTH 
		// }
...
linkObj.resolve = function(link, base, options)
{
	...
	if (options.acceptedSchemes[ resolvedUrl_parsed.extra.protocolTruncated ] === true)
	{
		...
		linkObj.relation(link, undefined, options);
	}
